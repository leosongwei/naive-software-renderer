...
(defun draw-triangle-gouraud (triangle c0 c1 c2 z-map pixels &optional (w *w*) (h *h*))
  (declare ;;(optimize (speed 3) (safety 0) (debug 0))
   (type (simple-array single-float (* *)) z-map)
   (type (simple-array single-float (3)) c0 c1 c2))
  (block :draw
    (let* ((vertices (triangle-vertices triangle))
           (v0-ndc (vertex-ndc (aref vertices 0)))
           (v1-ndc (vertex-ndc (aref vertices 1)))
      ...
      (mvb-let* ((x0 y0 z0 (ndc-xyz v0-ndc w h))
                ...
                (declare (type (signed-byte 16) x0 y0 x1 y1 x2 y2)
                         (type single-float z0 z1 z2))
        (if (> y0 y1) (progn (swap x0 x1) (swap y0 y1) (swap z0 z1) (swap c0 c1)))
        (if (> y0 y2) (progn (swap x0 x2) (swap y0 y2) (swap z0 z2) (swap c0 c2)))
        (if (> y1 y2) (progn (swap x1 x2) (swap y1 y2) (swap z1 z2) (swap c1 c2)))
        (if (or (= y0 y2)
                (= x0 x1 x2)) (return-from :draw))
        (let* ((height-total (- y2 y0))
               (zi0 (float (/ 1.0 z0)))
          ...
          (let* ((height (- y1 y0)))
            ...
            (dotimes (i height)
              (let* ((y (+ y0 i))
                     ...
                     (zi2p (float (+ (* zi0 (- 1.0 t2))
                                     (* zi2 t2))))
                     (c1p (interpolate-vec3 c0 c1 t1))
                     (c2p (interpolate-vec3 c0 c2 t2))
                          ...
                          (let ((z (float (/ 1.0 zi)))
                            ...
                            (if (< z z-map-depth)
                                (progn
                                  (setf (aref z-map x y) z)
                                  (setf (cffi:mem-aref pixels :unsigned-int (dot-offset x y w))
                                        (vec3-int-color color))))
                            (setf color (vec3+ color dcolor))
                            (incf zi dzi)
                            (incf x)))))))))
          ;; draw bottom
          (let* ((height (- y2 y1))
